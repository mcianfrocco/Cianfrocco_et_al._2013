...
def hsymVols(volodd,voleve,volall,hparams):
	...
	isearch = hparams['isearch']
	osearch = hparams['osearch']
	seam = hparams['seam']
	vert = hparams['vert']
	...
	rot,rise,vrot,vrise = findHsym(volall,hfile,apix,isearch,osearch,vert)

	# apply symmetry to all volumes
	volodd = applyHsym(volodd,rot,rise,apix,vrot,vrise,seam)
	voleve = applyHsym(voleve,rot,rise,apix,vrot,vrise,seam)
	volall = applyHsym(volall,rot,rise,apix,vrot,vrise,seam)
...
#===========================
def applyHsym(vol,rot,rise,apix,vrot=None,vrise=None,seam=False):
	"""
	...
	if seam is True:
		...
		# otherwise apply helical symmetry
		sumvol = vol.helicise(apix,rise,rot)

	# for vertical symmetry
	if vrot is not None and vrise is not None:
		vertvol = sumvol.helicise(apix,vrise,vrot)
		vertvol.process_inplace("normalize")
		return vertvol
...
def readHsym(hfile):
	...
	if len(pars) == 2:
		return float(pars[0]),float(pars[1]),None,None
	if len(pars) == 4:
		return float(pars[0]),float(pars[1]),float(pars[2]),float(pars[3])
	else:
		printError("Error: last line of hfile must be helical parameters")

#===========================
def applyHsym_seam(vol,wedgemask,hfile,apix):
	"""
